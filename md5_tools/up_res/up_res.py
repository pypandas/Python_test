...
def zip_unzip():
    ...
    files = os.listdir('./')
    for file in files:
        if file.split('.')[-1] == 'zip' and file != 'lua.zip' and file != 'resources.zip':
            ...
            # 创建resources文件夹
            if not os.path.exists(os.path.join(file.split('.')[0], 'resources')):
                os.mkdir(os.path.join(file.split('.')[0], 'resources'))
            else:
                shutil.rmtree(os.path.join(file.split('.')[0], 'resources'))
                os.mkdir(os.path.join(file.split('.')[0], 'resources'))

            # 将除了lua和version.xml之外的文件和文件夹移至resources
            files2 = os.listdir(os.path.join('./', file.split('.')[0]))
            for file2 in files2:
                if file2.split('.')[-1] == 'xml':
                    ...
                    try:
                        shutil.move(os.path.join(file.split('.')[0], file2),
                                    os.path.join(file.split('.')[0], 'resources'))
                    except:
                        if os.path.exists(os.path.join(file.split('.')[0], file2)):
                            shutil.rmtree(os.path.join(file.split('.')[0], file2))
                        else:
                            os.remove(os.path.join(file.split('.')[0], file2))

            # 压缩lua文件夹
            if os.path.exists(os.path.join(file.split('.')[0], 'lua')):
                zipFile_lua = zipfile.ZipFile('./lua.zip', 'w')
                zip_file_list_lua = []
                get_zip_file(os.path.join(file.split('.')[0], 'lua'), zip_file_list_lua)
                for file_lua in zip_file_list_lua:
                    zipFile_lua.write(file_lua)
                zipFile_lua.close()
            else:
                print(os.path.join(file.split('.')[0]), '非lua游戏')

            # 压缩resources文件夹
            zipFile_res = zipfile.ZipFile('./resources.zip', 'w')
            zip_file_list_res = []
            get_zip_file(os.path.join(file.split('.')[0], 'resources'), zip_file_list_res)
            for file_res in zip_file_list_res:
                zipFile_res.write(file_res)
            zipFile_res.close()

            # 删除lua和resources文件夹
            shutil.rmtree(os.path.join(file.split('.')[0]))
            os.remove(file)
            print(os.path.join(file.split('.')[0]), '资源更新完成')
